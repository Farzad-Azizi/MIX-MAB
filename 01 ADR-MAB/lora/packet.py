...
class myPacket():
    ...
    def __init__(self, nodeid, bsid, dist, transmitParams, logDistParams, sensi, setActions, nrActions, sfSet, prob, npacketsTransmitted, npacketsSuccessful): #choosenAction):
        self.nodeid = nodeid
        self.bsid = bsid
        self.dist = dist
    ...
    def updateTXSettings(self, bsDict, logDistParams, prob, npacketsTransmitted, npacketsSuccessful):
        ...
        """
        self.npacketsTransmitted = npacketsTransmitted
        self.npacketsSuccessful = npacketsSuccessful
        self.packetNumber += 1
        self.prob = prob

        if min(self.npacketsTransmitted)<=np.log(100/(1/self.nrActions)):
        #np.log(self.nrActions/(1/200000)):
             self.choosenAction = self.npacketsTransmitted.index(min(self.npacketsTransmitted))
        else:
             self.choosenAction = random.choice(self.nrActions, p=self.prob)
             #self.choosenAction = random.choice([random.randint(0,self.nrActions),random.choice(self.nrActions, p=self.prob)], p=[0.05,0.95])
             
        self.sf, self.freq, self.pTX = self.setActions[self.choosenAction]
        self.pRX = getRXPower(self.pTX, self.dist, logDistParams)
        #print(self.sf, self.pTX)
