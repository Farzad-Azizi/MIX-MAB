            ...
            if self.info_mode in ["NO", "PARTIAL"]:
                ...
                # with ACK -> 1, no ACK -> 0 
                    if self.ack:
                            ...
                            if prob[self.packets[0].choosenAction]>0:
                                reward[self.packets[0].choosenAction] =(npacketsSuccessful[self.packets[0].choosenAction]/npacketsTransmitted[self.packets[0].choosenAction])/prob[self.packets[0].choosenAction]
                            ...
                            for j in range(0, self.nrActions):
                                if npacketsSuccessful[j]!=0:     
                                  prob[j] =(1-self.learning_rate) * (weight[j]/(sum(weight)-prob.count(0))) + (self.learning_rate/self.nrActions)    
                           
                            if npacketsTransmitted[self.packets[0].choosenAction]  > np.log(self.nrActions/(1/200000)) and np.count_nonzero(prob)>1 and max(prob)> prob[self.packets[0].choosenAction]:
                                if (max(prob) - prob[self.packets[0].choosenAction])+.01 >= max(prob)/2:
                                # 2*np.sqrt((1/2*npacketsTransmitted[self.packets[0].choosenAction])*np.log((4*self.nrActions*np.power(npacketsTransmitted[self.packets[0].choosenAction],2))/(1/200000))) :
